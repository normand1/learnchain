...
Keep OpenAI keys and other secrets out of version control; load them via environment variables or untracked files. Let the app manage its global `app_config.toml`, avoiding manual edits while the TUI is running. Always prefer workspace-relative paths to maintain portability across environments.
